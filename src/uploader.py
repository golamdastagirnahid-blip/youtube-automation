...
class VideoUploader:
    ...
    def upload_video(self, video_info):
        ...
        if not self._verify_channel():
            ...
            print("   Authentication is broken or wrong account.")
            return None
        ...
        try:
            ...
            while response is None:
                try:
                    ...
                    if e.resp.status in [500, 502, 503, 504] and retry_count < max_retries:
                        ...
                        print(f"   Server error {e.resp.status}, retry {retry_count}/{max_retries} in {wait}s...")
                        time.sleep(wait)
                    else:
                        raise
            ...
            if thumb and os.path.exists(thumb):
                try:
                    ...
                    if "not verified" in str(e).lower():
                        print("Thumbnail: Channel not verified (need 100k+ subs or verification)")
                    else:
            ...
            if e.resp.status == 401:
                ...
                if "quotaExceeded" in error_msg:
                    ...
                    print(f"   CAUSE: Forbidden - {error_msg[:200]}")
